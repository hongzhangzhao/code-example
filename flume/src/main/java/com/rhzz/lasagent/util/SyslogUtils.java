...
public class SyslogUtils {
	...
	public static final String SYSLOG_MSG_RFC5424_0 = "(?:\\<(\\d{1,3})\\>)" + // priority
			"(?:(\\d?)\\s?)" + // version
			/*
	...
	public static final String SYSLOG_MSG_RFC3164_0 = "(?:\\<(\\d{1,3})\\>)" + "(?:(\\d)?\\s?)" + // version
	// stamp MMM d HH:mm:ss, single digit date has two spaces
			"([A-Z][a-z][a-z]\\s{1,2}\\d{1,2}\\s\\d{2}[:]\\d{2}[:]\\d{2})" + "\\s" + // separator
			"([\\w][\\w\\d\\.@-]*)" + // host
			"\\s(.*)$"; // body
	...
	public static String addFieldsToBody(Set<String> keepFields, String body, String priority, String version,
	...
	public SyslogUtils(boolean isUdp) {
		this(DEFAULT_SIZE, new HashSet<String>(Arrays.asList(SyslogSourceConfigurationConstants.DEFAULT_KEEP_FIELDS)),
				isUdp);
	}
	...
	public void addFormats(Map<String, String> formatProp) {
		...
		if (formatProp.containsKey(SyslogSourceConfigurationConstants.CONFIG_DATEFORMAT)) {
			fmt1.dateFormat
					.add(new SimpleDateFormat(formatProp.get(SyslogSourceConfigurationConstants.CONFIG_DATEFORMAT)));
		}
		formats.add(0, fmt1);
	}
	...
	private void initHeaderFormats() {
		...
		// 'Z' in timestamp indicates UTC zone, so replace it it with '+0000' for date
		// formatting
		...
		// FLUME-2497: SimpleDateFormat does not handle microseconds, Truncate after 3
		// digits.
	...
	public enum SyslogStatus {
		OTHER("Unknown"), INVALID("Invalid"), INCOMPLETE("Incomplete");
					...
					if (value != null) {
						...
						for (int dt = 0; dt < fmt.dateFormat.size(); dt++) {
							try {
								...
								/*
								 * Some code to try and add some smarts to the year insertion. Original code
								 * just added the current year which was okay-ish, but around January 1st
								 * becomes pretty na茂ve. The current year is added above. This code, if the year
								 * has been added does the following: 1. Compute what the computed time, but one
								 * month in the past would be. 2. Compute what the computed time, but eleven
								 * months in the future would be. If the computed time is more than one month in
								 * the future then roll it back a year. If the computed time is more than eleven
								 * months in the past then roll it forward a year. This gives us a 12 month
								 * rolling window (11 months in the past, 1 month in the future) of timestamps.
								 */
								if (fmt.addYear) {
	...
	public Event extractEvent(ChannelBuffer in) {
		...
		/*
		 * for protocol debugging ByteBuffer bb = in.toByteBuffer(); int remaining =
		 * bb.remaining(); byte[] buf = new byte[remaining]; bb.get(buf);
